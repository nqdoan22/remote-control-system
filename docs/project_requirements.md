...
- **File Transfer (Quản lý tệp tin):**
  ...
  - **Ràng buộc (Sandboxing):** Chỉ được phép thao tác bên trong thư mục sandbox (cấu hình trong `config.py`, mặc định `~/AgentSandbox`). Mọi nỗ lực thoát khỏi thư mục này đều bị từ chối.
